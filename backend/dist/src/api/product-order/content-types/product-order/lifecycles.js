...
module.exports = {
    ...
    async afterCreate(event) {
        ...
        const customerEmail = (_a = buyer.customerEmail) === null || _a === void 0 ? void 0 : _a.trim();
        ...
        const getEmailTemplate = (title, introText) => `
        ...
        emailTasks.push(strapi.plugins.email.services.email.send({
            to: 'admin@gmail.com',
            subject: `New order - ${order.orderNumber}`,
            html: getEmailTemplate(`New Order: ${escapeHtml(order.orderNumber)}`, 'A new order has been placed on the site.'),
        }));
        ...
        if (customerEmail) {
            emailTasks.push(strapi.plugins.email.services.email.send({
                ...
                html: getEmailTemplate(`Thank you for your order! (${escapeHtml(order.orderNumber)})`, `Hi ${escapeHtml(buyer.customerName || 'Customer')}, we have received your order and are currently processing it.`),
            }));
        }
