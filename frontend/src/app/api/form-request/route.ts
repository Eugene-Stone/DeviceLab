...
export async function POST(request: NextRequest) {
	...
	const { submitUrl, data: formData } = body;
	...
	const endpoint = submitUrl?.startsWith('/') ? submitUrl : `/${submitUrl}`;
	const targetLink = `${BACKEND_URL}/api${endpoint}`;

	// // Формируем заголовки: Authorization добавляем ТОЛЬКО если есть токен
